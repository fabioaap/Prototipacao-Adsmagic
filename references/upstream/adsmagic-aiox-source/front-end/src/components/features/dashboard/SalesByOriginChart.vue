    ...
    pie: {
      donut: {
        ...
        labels: {
          ...
          total: {
            show: true,
            label: t('dashboard.charts.totalSales'),
            fontSize: '14px',
            fontWeight: 600,
...
const chartSeries = computed(() => {
  ...
  return performance.map(item => item.sales)
})
...
const totalSales = computed(() => {
  return chartSeries.value.reduce((sum, val) => sum + val, 0)
})
</script>
...
<template>
  <Card>
    <CardHeader>
      <CardTitle>{{ t('dashboard.charts.salesByOrigin') }}</CardTitle>
      <p class="text-sm text-muted-foreground">
        {{ t('dashboard.charts.totalSales') }}: {{ totalSales }}
      </p>
    </CardHeader>
    <CardContent>
