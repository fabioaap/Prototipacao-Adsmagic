    ...
    <div class="max-w-md w-full mx-4">
      ...
      <div v-if="isLoading" class="text-center">
        ...
        <h2 class="section-title-md mb-2">
          {{ $t('integrations.google.callback.processing') }}
        </h2>
        <p class="section-description">
          {{ $t('integrations.google.callback.connecting') }}
        </p>
      </div>
      ...
      <div v-else-if="isSuccess" class="text-center">
        ...
        <h2 class="section-title-md mb-2">
          {{ $t('integrations.google.callback.success') }}
        </h2>
        <p class="section-description mb-6">
          {{ $t('integrations.google.callback.connected') }}
        </p>
        <Button @click="goToIntegrations" class="w-full">
          {{ $t('integrations.google.callback.continue') }}
        </Button>
      </div>
      ...
      <div v-else-if="error" class="text-center">
        ...
        <h2 class="section-title-md mb-2">
          {{ $t('integrations.google.callback.error') }}
        </h2>
        <p class="section-description mb-6">
          {{ error }}
        </p>
        <div class="flex space-x-3">
          <Button variant="outline" @click="retry" class="flex-1">
            {{ $t('integrations.google.callback.retry') }}
          </Button>
          <Button @click="goToIntegrations" class="flex-1">
            {{ $t('integrations.google.callback.back') }}
          </Button>
        </div>
      </div>
    </div>
...
const processCallback = async () => {
  try {
    ...
    await integrationsStore.handleOAuthCallback('google', code)
    
    isSuccess.value = true
    success('Google Ads conectado!', 'Sua conta do Google Ads foi conectada com sucesso.')
  } catch (err) {
