...
handoffs:
  - label: "→ Arquitetura (Architect)"
    agent: architect
    prompt: "Integre as decisões de dados acima com a arquitetura da solução."
  - label: "→ Implementar (Dev)"
    agent: dev
    prompt: "Implemente no código as mudanças de dados descritas acima."
  - label: "→ Revisão (QA)"
    agent: qa
    prompt: "Revise riscos, cobertura e regressões para as mudanças de dados acima."
---
