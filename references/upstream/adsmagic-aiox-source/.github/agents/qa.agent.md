...
handoffs:
  - label: "→ Corrigir (Dev)"
    agent: dev
    prompt: "Aplique as correções apontadas na revisão acima e revalide com as skills aplicáveis."
  - label: "→ Deploy (DevOps)"
    agent: devops
    prompt: "As validações passaram; prossiga com operações de repositório ou deploy para a solução acima."
---
...
- Se houver falhas, devolva o trabalho ao agente `dev` com feedback específico.
- Se o código estiver pronto para operações Git remotas, envolva o agente `devops`.
