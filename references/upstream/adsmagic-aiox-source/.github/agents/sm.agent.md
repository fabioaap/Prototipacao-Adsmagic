...
handoffs:
  - label: "→ Validar Story (PO)"
    agent: po
    prompt: "Valide a story acima, critérios de aceite e prontidão para implementação."
  - label: "→ Implementar (Dev)"
    agent: dev
    prompt: "Implemente a story acima seguindo os critérios definidos."
---
...
- Depois de criar a story, passe a validação para o agente `po`.
