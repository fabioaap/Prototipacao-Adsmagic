-- Migration 063: Conversões por regra de etapa/origem
-- Data: 2026-02-22
-- Descrição:
--   Cria trigger em contacts (INSERT/UPDATE de current_stage_id) para gerar
--   conversion_events com base em stages.event_config.routes.
--
-- Estrutura esperada em stages.event_config:
-- {
--   "defaultValue": 0,
--   "defaultCurrency": "BRL",
--   "routes": [
...
--       "sourceOriginId": "uuid-origem-opcional",
--       "priority": 0,
--       "integrationAccountId": "uuid-opcional",
--       "conversionActionId": "string-opcional",
--       "value": 0,
--       "currency": "BRL"
--     }
--   ]
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      v_event_type := COALESCE(NULLIF(v_route->>'eventType', ''), NULLIF(v_route->>'event_type', ''), 'lead');
      ...
      IF NULLIF(COALESCE(v_route->>'integrationAccountId', v_route->>'integration_account_id'), '') IS NOT NULL
        AND COALESCE(v_route->>'integrationAccountId', v_route->>'integration_account_id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      THEN
        v_integration_account_id := COALESCE(v_route->>'integrationAccountId', v_route->>'integration_account_id')::uuid;
      END IF;
