/**
 * Edge Function para API de Origens
 * 
 * Router principal que gerencia todas as operações de origens:
 * - GET /origins - Listar origens (system + custom do projeto)
 * - GET /origins/:id - Obter origem específica
 * - POST /origins - Criar origem customizada
 * - PATCH /origins/:id - Atualizar origem customizada
 * - DELETE /origins/:id - Deletar origem customizada
 * 
 * Origens system (type = 'system') não podem ser criadas, alteradas ou deletadas.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[Origins Edge Function]', {
      method: req.method,
      path: url.pathname,
      pathParts,
      userId: user.id
    ...
    // DELETE /origins/:id - Deletar origem
    ...
    console.error('[Origins Edge Function Error]', error)
