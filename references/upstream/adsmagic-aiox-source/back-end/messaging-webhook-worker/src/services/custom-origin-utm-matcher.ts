/**
 * Custom Origin UTM Matcher — REST client version
 */
...
export async function findCustomOriginByUtmSourceMatch(params: {
  ...
  const { supabase, projectId, utmSource } = params
  ...
  const rows = await supabase.select(
    'origins',
    {
      ...
      'type': `eq.custom`,
      'is_active': `eq.true`,
      'utm_source_match_value': `not.is.null`,
    },
    'id,utm_source_match_mode,utm_source_match_value,created_at'
  )

  if (!rows || rows.length === 0) return null
  return resolveBestOriginUtmSourceMatch(rows as unknown as OriginUtmMatchRuleRow[], normalizedSource)
}
