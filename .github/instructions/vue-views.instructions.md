---
description: "Use when creating or editing Vue feature screens, module pages, or routed views under Plataforma/src/views. Covers page structure, routing expectations, and Tailwind usage for this prototype app."
name: "Vue Views Guidelines"
applyTo: "Plataforma/src/views/**/*.vue"
---

# Vue Views Guidelines

- Treat files in `Plataforma/src/views/` as routed screen content only. Layout chrome belongs to `Plataforma/src/components/layout/`, so do not import or recreate `AppLayout` inside a view.
...
- When a task adds a new screen, also register it as a lazy-loaded child route in `Plataforma/src/router/index.ts` and set `meta.title` so the document title stays correct.
